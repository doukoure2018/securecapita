...
public class UserController {
    ...
    public ResponseEntity<HttpResponse> saveUser(@RequestBody @Valid UserDto userDto) throws InterruptedException {
        ...
        return ResponseEntity.created(getUri()).body(
                HttpResponse.builder()
                        ...
                        .message(String.format("User account created for user %s", userResponse.getFirstName()))
                        .status(CREATED)
    ...
    public ResponseEntity<HttpResponse> login(@RequestBody @Valid LoginDto loginDto){
        ...
        return Optional.ofNullable(userResponse.getUsingMfa()).orElse(false)
                                         ? sendVerificationCode(userResponse)
    ...
    public ResponseEntity<HttpResponse> profile(Authentication authentication){
        UserResponse userResponse = userService.getUserByEmail(getAuthenticatedUser(authentication).getEmail());
        UserResponse loggedInUser = getLoggedInUser(authentication);
        assert loggedInUser != null;
        //userResponse.setPhone(loggedInUser.getPhone());
        userResponse.setRoleName(loggedInUser.getRoleName());
        userResponse.setPermissions(loggedInUser.getPermissions());
    ...
    public ResponseEntity<HttpResponse> updateUser(@RequestBody @Valid UpdateForm updateForm){
        UserResponse updateUserDto = userService.updateUserDetails(updateForm);
    ...
    public ResponseEntity<HttpResponse> verifyCode(
        ...
        UserResponse userDto=userService.verifyCode(email,code);
    ...
    {
        TimeUnit.SECONDS.sleep(3);
        UserResponse userDto = userService.verifyPasswordKey(key);
        return ResponseEntity.ok().body(
                HttpResponse.builder()
                        .timeStamp(now().toString())
    ...
    public ResponseEntity<HttpResponse> updatePassword(Authentication authentication, @RequestBody @Valid UpdatePasswordForm form) {
        UserResponse userResponse = getAuthenticatedUser(authentication);
    ...
    public ResponseEntity<HttpResponse> updateUserRole(Authentication authentication, @PathVariable("roleName") String roleName) {
        UserResponse userDTO = getAuthenticatedUser(authentication);
    ...
    public ResponseEntity<HttpResponse> toggleMfa(Authentication authentication) throws InterruptedException {
        TimeUnit.SECONDS.sleep(3);
        UserResponse user = userService.toggleMfa(getAuthenticatedUser(authentication).getEmail());
    ...
    public ResponseEntity<HttpResponse> updateProfileImage(Authentication authentication, @RequestParam("image") MultipartFile image) throws InterruptedException {
        UserResponse user = getAuthenticatedUser(authentication);
    ...
    private ResponseEntity<HttpResponse> sendVerificationCode(UserResponse userDto){
    ...
    private ResponseEntity<HttpResponse> sendResponse(UserResponse userDto){
    ...
    private UserPrincipal getUserPrincipal(UserResponse user) {
        UserResponse userResponse = userService.getUserByEmail(user.getEmail());
        return new UserPrincipal(toUser(userResponse), rolesService.getRoleByUserId(userResponse.getId()));
    }
